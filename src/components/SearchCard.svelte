<script lang="ts">
  ...
  const fetchData = async () => {
    ...
    const yesterday = moment()
      ...
      .startOf("day")
      .format("YYYY-MM-DD");
    const url = `https://api.covid19api.com/country/${bindingCountry}/status/confirmed?from=${yesterday}&to=${today}`;
...
>
  ...
  <div class="">
    <input
      bind:value={bindingCountry}
      on:change={fetchData}
      placeholder="Add Country"
      class="p-2 rounded-md border-solid border-2 border-black"
    />
